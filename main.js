...
fetch(BREEDS_URL)
    ...
    .then(data => {
        ...
        for (let i = 0; i < breedsArray.length; i++){
            ...
            option.value = breedsArray[i] //<option value = 'breed iteration'>
    ...
    select.addEventListener('change', event => {
        ...
        doggoInfo.assignDisikes()
        doggoInfo.assignFact()
    })//when an option changes in the box, then we want to do something
    ...
    const doggoInfo = {
        ...
        assignMF(){
            x = (Math.floor(Math.random() * 3) === 0) // we get a number from 0 to 1. If its 0, its true, if its 1 its false
            if(x) {
                this.MF = 'Female';
        ...
        assignAge() {
            this.age = Math.floor(Math.random() * 16 + 1)//this plus one returns no 0, just minimum 1 to 16
